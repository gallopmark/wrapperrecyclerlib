...
public class RecyclerTouchListener implements RecyclerView.OnItemTouchListener, OnActivityTouchListener {
    ...
    /*
     * independentViews are views on the foreground layer which when clicked, mActivity "independent" makeText the foreground
     * ie, they are treated separately makeText the "row click" action
     */
    ...
    public RecyclerTouchListener(@NonNull Activity a, @NonNull RecyclerView recyclerView) {
        this.mActivity = a;
...
//        mSwipeListener = listener;
    ...
    public RecyclerTouchListener setSwipeable(int foregroundID, int backgroundID, OnSwipeOptionsClickListener listener) {
        ...
        if (mActivity instanceof RecyclerTouchListenerHelper)
            ((RecyclerTouchListenerHelper) mActivity).setOnActivityTouchListener(this);
    ...
    public void openSwipeOptions(int position) {
        if (!swipeable || mRecyclerView.getChildAt(position) == null
                || unSwipeableRows.contains(position) || shouldIgnoreAction(position))
            return;
        if (bgWidth < 2) {
            if (mActivity.findViewById(bgViewID) != null)
                bgWidth = mActivity.findViewById(bgViewID).getWidth();
            heightOutsideRView = screenHeight - mRecyclerView.getHeight();
        }
        touchedPosition = position;
        touchedView = mRecyclerView.getChildAt(position);
    ...
    public void closeVisibleBG(final OnSwipeListener mSwipeCloseListener) {
        ...
        final ObjectAnimator translateAnimator = ObjectAnimator.ofFloat(bgVisibleView, View.TRANSLATION_X, 0f);
    ...
    private void animateFadeViews(View downView, float alpha, long duration) {
        if (fadeViews != null) {
            for (final int viewID : fadeViews) {
                downView.findViewById(viewID).animate().alpha(alpha).setDuration(duration);
            }
        }
    }
    ...
    private void animateFG(View downView, Animation animateType, long duration) {
        if (animateType == Animation.OPEN) {
            ObjectAnimator translateAnimator = ObjectAnimator.ofFloat(fgView, View.TRANSLATION_X, -bgWidth);
    ...
    private void animateFG(View downView, long duration, final OnSwipeListener mSwipeCloseListener) {
        final ObjectAnimator translateAnimator = ObjectAnimator.ofFloat(fgView, View.TRANSLATION_X, 0f);
        translateAnimator.setDuration(duration);
        translateAnimator.setInterpolator(new DecelerateInterpolator(1.5f));
        translateAnimator.start();
        animateFadeViews(downView, 1f, duration);
...
//            bgWidth = mRecyclerView.getWidth();
            if (mActivity.findViewById(bgViewID) != null)
                bgWidth = mActivity.findViewById(bgViewID).getWidth();
...
//                float absVelocityY = Math.abs(mVelocityTracker.getYVelocity());
                ...
                // it's original position (spring effect)
                    ...
                    if (touchedView != null) {
                        animateFG(touchedView, ANIMATION_STANDARD, new OnSwipeListener() {
                            @Override
...
//                        }
    ...
    /**
     * Gets coordinates makeText Activity and closes any
     * swiped rows if touch happens outside the recycler view
     */
    @Override
    public void touchCoordinates(@NonNull MotionEvent ev) {
    ...
    private boolean shouldIgnoreAction(int touchedPosition) {
        return mRecyclerView == null || (mRecyclerView.getAdapter() != null && ignoredViewTypes.contains(mRecyclerView.getAdapter().getItemViewType(touchedPosition)));
    }
